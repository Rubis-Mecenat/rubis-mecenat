<article id = "post-<?php the_ID(); ?>" <?php post_class('entry has-divider' . ($post_format ? ' ' . $post_format . '-entry' : '')); ?>>
	...
	if(strpos($content , 'http://') === 0){
		...
		$first_link = substr($content , 0 , $end);
		$content = substr($content , $end);
		$content = apply_filters('the_content', $content);
		
		/* youtube video */
		if(strpos($first_link , 'youtube.com')){
			$components = parse_url($first_link);
			parse_str($components['query'] , $vars);
			$video_id = $vars['v'];
			$video_sc = '[youtube video_id = "' . $video_id . '" size = "featured"]';
		}else
		/* vimeo video */
		if(strpos($first_link , 'vimeo.com')){
			preg_match('/vimeo\.com\/([0-9]*)/' , $first_link , $matches);
			$video_id = $matches[1];
			$video_sc = '[vimeo video_id = "' . $video_id . '" size = "featured"]';
		}else
		/* dailymotion video */
		if(strpos($first_link , 'dailymotion.com')){
			$video_id = strtok(basename($first_link), '_');
			$video_sc = '[dailymotion video_id = "' . $video_id . '" size = "featured"]';
		}
	}
	?>
	...
	<div class = "entry-feature">
		<div class = "video-wrap">
			<?php echo do_shortcode($video_sc); ?>
		</div><!-- .video-wrap -->
	</div><!-- .entry-feature -->
	<?php endif; ?>
